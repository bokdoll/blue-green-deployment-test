pipeline {
  ...
  stages {
    ...
    stage('Deploy') {
      steps {
        sh 'aws elasticbeanstalk create-application-version --region ap-northeast-2 --application-name blue-green-test devion-label jenkins-deploy --source-bundle S3Bucket="jenkins-beanstalk-deployment",S3Key="application.zip"'
